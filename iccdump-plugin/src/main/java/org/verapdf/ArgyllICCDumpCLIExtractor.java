...
public class ArgyllICCDumpCLIExtractor extends AbstractICCProfileFeaturesExtractor {
	...
	public List<FeatureTreeNode> getICCProfileFeatures(ICCProfileFeaturesData iccProfileFeaturesData) {
		...
		try {
			res = execCLI();
		} catch (IOException | InterruptedException | FeatureParsingException e) {
			LOGGER.error(e);
		}
		try {
			clean();
	...
	private List<FeatureTreeNode> execCLI() throws IOException, InterruptedException, FeatureParsingException {
